...
import { LayoutDashboard, Users, ClipboardList, Flame, Droplets, DollarSign, CircleAlert as AlertCircle, Plus, CircleCheck as CheckCircle, Trash2, Settings, ChartBar as BarChart3, ChartPie as PieChart, Search, Cloud, ChevronRight, Minus, LogOut, Lock, Store, CircleArrowUp as ArrowUpCircle, CircleArrowDown as ArrowDownCircle, MapPin, UserCheck, Package, ShoppingCart, ShoppingBag, Download, CreditCard as Edit, Clock, Calendar } from 'lucide-react';
...
function LavOSMain() {
  ...
  // REFS AUTO-FOCO MOSTRADOR (VENTAS)

  const searchInputRef = useRef(null);

  const empSelectRef = useRef(null);

  const srvSelectRef = useRef(null);
  ...
  // REFS AUTO-FOCO GLOBAL

  const ordersSearchRef = useRef(null);

  const supplySearchRef = useRef(null);

  const deliverEmpSelectRef = useRef(null);

  const deliverSubmitRef = useRef(null);
  ...
  const supplyStockRef = useRef(null);

  const supplyMinRef = useRef(null);

  const restockQtyRef = useRef(null);

  const restockCostRef = useRef(null);

  

  // REFS CAJA/MOVIMIENTOS

  const txTypeRef = useRef(null);

  const txDateRef = useRef(null);

  const txCategoryRef = useRef(null);

  const txAmountRef = useRef(null);

  const txDescRef = useRef(null);

  const txSubmitRef = useRef(null);



  const modalTxDateRef = useRef(null);

  const modalTxCategoryRef = useRef(null);

  const modalTxAmountRef = useRef(null);

  const modalTxDescRef = useRef(null);
  ...
  const saveToCloud = async (key, value) => { 
    ...
    try { 
      ...
      await setDoc(doc(db, 'negocios', storeId, 'datos', 'estado_maestro'), { [key]: value }, { merge: true }); 
  ...
  const [selectedSupplyIdx, setSelectedSupplyIdx] = useState(0);



  const [modals, setModals] = useState({ client: false, deliverOrder: false, employee: false, service: false, transaction: false, supply: false, category: false, restockSupply: false, wipeData: false, editOrder: false, deleteTx: false, deleteOrder: false, viewOrder: false, editCategory: false, editService: false, editClient: false, editSupply: false });
  ...
  const [forms, setForms] = useState({
    ...
    transaction: { id: null, type: 'SALIDA', amount: '', category: '', desc: '', date: getLocalDateString() },
  ...
  // --- FILTROS Y LÓGICA INTELIGENTE DE ORDENAMIENTO DE FECHAS ---
  ...
  const sortedAndFilteredOrders = useMemo(() => {
    ...
    let list = _orders.filter(o => {
      ...
      // Activas se ven siempre. El historial SOLO MUESTRA si la fecha de ENTREGA coincide.

      if (activeOrderTab === 'activas') return o.status !== 'Entregado';

      return o.status === 'Entregado' && isDateInRange(o.deliveryDate || o.requestDate);
    ...
    return list.filter(o => {
      ...
      if (activeOrderTab === 'activas') {

        return (w[b.priority] - w[a.priority]) || (new Date(a.requestDate).getTime() - new Date(b.requestDate).getTime());

      } else {

        return new Date(b.deliveryDate || b.requestDate).getTime() - new Date(a.deliveryDate || a.requestDate).getTime();
  ...
  // --- ATAJOS DE TECLADO Y AUTO-FOCO MAESTRO ---
    ...
    if (currentUser?.role === 'employee') {
      ...
      const handleGlobalKey = (e) => {
        ...
        if (e.altKey) {
          ...
          if (e.key === '1' || e.code === 'Digit1' || e.code === 'Numpad1') { e.preventDefault(); setActiveTab('mostrador'); }

          if (e.key === '2' || e.code === 'Digit2' || e.code === 'Numpad2') { e.preventDefault(); setActiveTab('ordenes'); }

          if (e.key === '3' || e.code === 'Digit3' || e.code === 'Numpad3') { e.preventDefault(); setActiveTab('finanzas'); }

          if (e.key === '4' || e.code === 'Digit4' || e.code === 'Numpad4') { e.preventDefault(); setActiveTab('insumos'); }

          if (e.key.toLowerCase() === 'f' || e.code === 'KeyF') { e.preventDefault(); dateFilterRef.current?.focus(); }
          ...
          if (e.key.toLowerCase() === 'b' || e.code === 'KeyB') {
            ...
            if (activeTab === 'mostrador') searchInputRef.current?.focus();

            if (activeTab === 'ordenes') ordersSearchRef.current?.focus();

            if (activeTab === 'insumos') supplySearchRef.current?.focus();
          ...
          if (activeTab === 'ordenes') {
             ...
             if (e.key.toLowerCase() === 'a' || e.code === 'KeyA') { e.preventDefault(); setActiveOrderTab('activas'); setSelectedOrderIdx(0); }

             if (e.key.toLowerCase() === 'h' || e.code === 'KeyH') { e.preventDefault(); setActiveOrderTab('historial'); setSelectedOrderIdx(0); }
  ...
  useEffect(() => {
    ...
    if (currentUser?.role === 'employee') {
      ...
      if (activeTab === 'mostrador' && !orderForm.clientId) setTimeout(() => searchInputRef.current?.focus(), 300);

      if (activeTab === 'ordenes') { setTimeout(() => ordersSearchRef.current?.focus(), 300); setSelectedOrderIdx(0); }

      if (activeTab === 'finanzas') setTimeout(() => txTypeRef.current?.focus(), 300);

      if (activeTab === 'insumos') { setTimeout(() => supplySearchRef.current?.focus(), 300); setSelectedSupplyIdx(0); }

    }

  }, [activeTab, currentUser, orderForm.clientId]);



  // --- NAVEGACIÓN EN ÓRDENES POR TECLADO (SCROLL AUTO) ---

  useEffect(() => { setSelectedOrderIdx(0); }, [activeOrderTab, orderSearchTerm]);

  useEffect(() => {

    if (activeTab === 'ordenes' && currentUser?.role === 'employee') {

      const handleOrderSelection = (e) => {

        if (Object.values(modals).some(v => v === true)) return; 

        if (document.activeElement === ordersSearchRef.current && (e.key === 'ArrowDown' || e.key === 'ArrowUp')) {

          e.preventDefault();

          ordersSearchRef.current?.blur();

        }

        

        let nextIdx = selectedOrderIdx;

        if (e.key === 'ArrowRight' || e.key === 'ArrowDown') { e.preventDefault(); nextIdx = Math.min(selectedOrderIdx + 1, sortedAndFilteredOrders.length - 1); }

        if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') { e.preventDefault(); nextIdx = Math.max(selectedOrderIdx - 1, 0); }

        

        if (nextIdx !== selectedOrderIdx) {

          setSelectedOrderIdx(nextIdx);

          document.getElementById(`order-card-${nextIdx}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });

        }



        if (e.key === 'Enter' && document.activeElement !== ordersSearchRef.current) {

          e.preventDefault();

          const o = sortedAndFilteredOrders[selectedOrderIdx];

          if (o && o.status !== 'Entregado') requirePriceOrExecute(o, executeDeliver);

        }

      };

      window.addEventListener('keydown', handleOrderSelection);

      return () => window.removeEventListener('keydown', handleOrderSelection);

    }

  }, [activeTab, currentUser, sortedAndFilteredOrders, selectedOrderIdx, modals]);



  // --- NAVEGACIÓN EN INSUMOS POR TECLADO (SCROLL AUTO) ---

  useEffect(() => { setSelectedSupplyIdx(0); }, [supplySearchTerm]);

  useEffect(() => {

    if (activeTab === 'insumos' && activeInvTab === 'insumos' && currentUser?.role === 'employee') {

      const handleSupplySelection = (e) => {

        if (Object.values(modals).some(v => v === true)) return; 

        if (document.activeElement === supplySearchRef.current && (e.key === 'ArrowDown' || e.key === 'ArrowUp')) {

          e.preventDefault();

          supplySearchRef.current?.blur();

        }

        

        const filtered = supplySearchTerm ? _supplies.filter(s => s.name.toLowerCase().includes(supplySearchTerm.toLowerCase())) : _supplies;

        let nextIdx = selectedSupplyIdx;

        

        if (e.key === 'ArrowRight' || e.key === 'ArrowDown') { e.preventDefault(); nextIdx = Math.min(selectedSupplyIdx + 1, Math.max(0, filtered.length - 1)); }

        if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') { e.preventDefault(); nextIdx = Math.max(selectedSupplyIdx - 1, 0); }

        

        if (nextIdx !== selectedSupplyIdx) {

          setSelectedSupplyIdx(nextIdx);

          document.getElementById(`supply-card-${nextIdx}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });

        }



        if (e.key === 'Enter' && document.activeElement !== supplySearchRef.current) {

          e.preventDefault();

          const s = filtered[selectedSupplyIdx];

          if (s) {

            vibrate();

            setForms(p => ({...p, restockSupply: {id: s.id, name: s.name, qty: '', cost: '', date: getLocalDateString()}}));

            toggleModal('restockSupply');

            setTimeout(() => restockQtyRef.current?.focus(), 100);
      ...
      window.addEventListener('keydown', handleSupplySelection);

      return () => window.removeEventListener('keydown', handleSupplySelection);

    }

  }, [activeTab, activeInvTab, currentUser, _supplies, selectedSupplyIdx, modals, supplySearchTerm]);
  ...
  const handleAddClient = () => {
    ...
    if (activeTab === 'mostrador') {
      ...
      setOrderForm(p => ({ ...p, clientId: newClient.id }));

      if (currentUser?.role === 'employee') setTimeout(() => empSelectRef.current?.focus(), 100);
  ...
  const handleSaveOrder = () => {
    ...
    const newOrder = { id: Date.now(), ...orderForm, total: parseFloat(orderForm.manualPrice || 0), status: 'Pendiente', isPaid: false, deliveredBy: null, deliveryDate: null };
    ...
    showMsg("Orden procesada con éxito", false);
  ...
  const requirePriceOrExecute = (order, executeFn) => {
    ...
    if (order.total === 0) {
      ...
      showMsg("⚠️ DEBES ASIGNAR EL PRECIO FINAL ANTES DE CONTINUAR", true);
  ...
  const executeDeliver = (order) => {
    ...
    if (currentUser?.role === 'employee') setTimeout(() => deliverEmpSelectRef.current?.focus(), 100);
  ...
  const confirmDelivery = () => {
    ...
    if (!forms.deliverOrder.deliveredBy) return showMsg("Selecciona quién entregó la orden");
    ...
    // Asegurar que se guarde como Transacción si no había sido pagada antes
    ...
    // Guardar fecha exacta de entrega

    setOrders(_orders.map(x => x.id === o.id ? { ...x, status: 'Entregado', isPaid: true, paidDate: getLocalDateString(), deliveredBy: parseInt(forms.deliverOrder.deliveredBy), deliveryDate: getLocalDateString() } : x));

    toggleModal('deliverOrder');

    showMsg("Cobro registrado y orden entregada", false);

    if (currentUser?.role === 'employee') setTimeout(() => ordersSearchRef.current?.focus(), 100);
  ...
  const handleUpdateOrderStatus = (id, newStatus) => { 
    ...
    if (order.status === 'Entregado' && newStatus !== 'Entregado') {
      ...
      setOrders(_orders.map(o => o.id === id ? { ...o, status: newStatus, isPaid: false, paidDate: null, deliveredBy: null, deliveryDate: null } : o));

      showMsg("Ingreso revertido y orden actualizada", false);
  ...
  const handleEditOrder = () => {
    ...
    if (originalOrder.status === 'Entregado' && forms.editOrder.status !== 'Entregado') {
       ...
       setOrders(_orders.map(o => o.id === forms.editOrder.id ? { ...o, total: newTotal, bags: forms.editOrder.bags, status: forms.editOrder.status, isPaid: false, paidDate: null, deliveredBy: null, deliveryDate: null } : o));

       showMsg("Orden revertida y modificada", false);
    ...
    if (currentUser?.role === 'employee' && activeTab === 'ordenes') setTimeout(() => ordersSearchRef.current?.focus(), 100);
  ...
  const handleAddTransaction = () => {
    ...
    if (!forms.transaction.amount || !forms.transaction.category) return showMsg("Faltan datos de caja");
    ...
    setForms(p => ({ ...p, transaction: { id: null, type: 'SALIDA', amount: '', category: _txCategories.out[0] || '', desc: '', date: getLocalDateString() } }));

    if(currentUser?.role === 'employee' && activeTab === 'finanzas') setTimeout(() => txTypeRef.current?.focus(), 100);
  ...
  const handleViewTicket = (desc) => {
    ...
    if(match && match[1]) {
      ...
      const order = _orders.find(o => o.id.toString().slice(-4) === match[1]);

      if(order) { setForms(p => ({...p, viewOrder: order})); toggleModal('viewOrder'); } 

      else showMsg("Ticket original no encontrado");
  ...
  const handleAddCategory = () => {
    ...
    showMsg("Categoría guardada", false);
  ...
  const handleEditCategory = () => {
    ...
    showMsg("Categoría y gastos actualizados", false);
  ...
  const handleAddSupply = () => {
    ...
    if(currentUser?.role === 'employee' && activeTab === 'insumos') setTimeout(() => supplySearchRef.current?.focus(), 100);
  ...
  const handleDeleteSupply = (s) => {
    ...
    if(window.confirm(`¿Borrar ${s.name}? Se borrará también su historial de compras en Movimientos.`)) {
  ...
  const handleDownloadCSV = () => {
    ...
    csvContent += `Fecha de reporte:,${formatDateDisplay(getLocalDateString())}\n\n`;
    ...
    filteredTransactions.forEach(t => {
      ...
      csvContent += `${formatDateDisplay(t.date)},${t.type},${t.category},${t.amount},"${safeDesc}"\n`;
    ...
    link.setAttribute("download", `Reporte_LavOS_Morales_${formatDateDisplay(getLocalDateString()).replace(/\//g, '-')}.csv`);
  ...
  const renderMostradorLaptop = () => {
    ...
    return (
      ...
      <div className="flex flex-col gap-6 h-full w-full animate-in fade-in duration-300">

        <div className="bg-slate-900 text-white p-4 rounded-2xl shadow-lg flex justify-between items-center shrink-0">

          <p className="font-bold text-sm"><span className="bg-blue-600 px-2 py-1 rounded text-xs mr-2">ATAJOS</span> Usa <kbd className="bg-slate-700 px-2 py-1 rounded">Enter</kbd> para avanzar y <kbd className="bg-slate-700 px-2 py-1 rounded">Alt + Número</kbd> para cambiar pestaña.</p>

          <p className="font-black text-blue-400 hidden md:block">Modo Laptop ⚡</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col">
            ...
            <h3 className="font-black text-slate-800 mb-4 flex items-center"><UserCheck className="w-5 h-5 mr-2 text-blue-600"/> 1. Cliente</h3>

            {!orderForm.clientId ? (

              <>

                <input ref={searchInputRef} type="text" placeholder="Escribe nombre y presiona Enter..." className="w-full bg-slate-50 p-4 rounded-xl font-bold border-2 border-blue-200 outline-none focus:border-blue-600 text-lg mb-2" value={posSearchTerm} onChange={e => setPosSearchTerm(e.target.value)} 

                  onKeyDown={e => { if(e.key === 'Enter' && topClient) { e.preventDefault(); setOrderForm({...orderForm, clientId: topClient.id}); setTimeout(() => empSelectRef.current?.focus(), 50); } }} />

                <div className="flex-1 overflow-y-auto max-h-64 no-scrollbar border rounded-xl">

                  {filteredClients.map((c, idx) => (<div key={c.id} className={`p-4 border-b last:border-0 font-bold truncate ${idx === 0 ? 'bg-blue-50 text-blue-800' : 'bg-white text-slate-600'}`}>{c.name}</div>))}

                  {filteredClients.length === 0 && <button onClick={() => toggleModal('client')} className="w-full p-4 bg-slate-100 text-blue-600 font-black">+ Crear Cliente</button>}
              ...
              <div className="bg-emerald-50 border-2 border-emerald-500 p-5 rounded-2xl flex justify-between items-center">

                <div className="truncate"><p className="font-black text-emerald-900 text-xl truncate pr-2">{_clients.find(c => c.id === orderForm.clientId)?.name}</p></div>

                <button onClick={() => {setOrderForm({...orderForm, clientId: ''}); setPosSearchTerm(''); setTimeout(() => searchInputRef.current?.focus(), 50);}} className="bg-white text-emerald-700 px-3 py-2 rounded-xl font-black text-xs shrink-0 shadow-sm">Cambiar</button>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col gap-6">
            ...
            <div>
              ...
              <h3 className="font-black text-slate-800 mb-3 flex items-center"><Users className="w-5 h-5 mr-2 text-purple-600"/> 2. Recibido Por</h3>

              <select ref={empSelectRef} className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-200 font-bold outline-none focus:border-blue-600 text-lg cursor-pointer" value={orderForm.receivedBy} onChange={e => setOrderForm({...orderForm, receivedBy: e.target.value})} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); srvSelectRef.current?.focus(); } }}>

                <option value="" disabled>Selecciona con flechas...</option>

                {_employees.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
            ...
            <div>
              ...
              <h3 className="font-black text-slate-800 mb-3 flex items-center"><Package className="w-5 h-5 mr-2 text-orange-600"/> 3. Servicio</h3>

              <select ref={srvSelectRef} className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-200 font-bold outline-none focus:border-blue-600 text-lg cursor-pointer" value={orderForm.serviceId} onChange={e => { const s = _servicesConfig.find(x => x.id === parseInt(e.target.value)); setOrderForm({...orderForm, serviceId: s?.id || '', manualPrice: s?.basePrice || 0}); }} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); prioritySelectRef.current?.focus(); } }}>

                <option value="" disabled>Selecciona el servicio...</option>

                {_servicesConfig.map(s => <option key={s.id} value={s.id}>{s.name} (Q{s.basePrice})</option>)}
            ...
            <div>
              ...
              <h3 className="font-black text-slate-800 mb-3 flex items-center"><AlertCircle className="w-5 h-5 mr-2 text-rose-500"/> Prioridad</h3>

              <select ref={prioritySelectRef} className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-200 font-bold outline-none focus:border-blue-600 text-lg cursor-pointer" value={orderForm.priority} onChange={e => setOrderForm({...orderForm, priority: e.target.value})} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); bagsInputRef.current?.focus(); } }}>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <h3 className="font-black text-slate-800 mb-3 text-center">Bolsas</h3>

                <input ref={bagsInputRef} type="number" className="w-full p-6 bg-slate-50 rounded-2xl border-2 border-slate-200 font-black text-4xl text-center outline-none focus:border-blue-600 num-font" placeholder="0" value={orderForm.bags} onChange={e => setOrderForm({...orderForm, bags: e.target.value})} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); priceInputRef.current?.focus(); } }} />

              </div>

              <div>

                <h3 className="font-black text-emerald-700 mb-3 text-center">Precio Q</h3>

                <input ref={priceInputRef} type="number" className="w-full p-6 bg-emerald-50 rounded-2xl border-2 border-emerald-200 font-black text-4xl text-center outline-none focus:border-emerald-600 text-emerald-700 num-font" placeholder="0" value={orderForm.manualPrice} onChange={e => setOrderForm({...orderForm, manualPrice: e.target.value})} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); submitBtnRef.current?.focus(); } }} />

              </div>

            </div>

            <button ref={submitBtnRef} onClick={handleSaveOrder} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); handleSaveOrder(); } }} className="w-full py-8 mt-auto rounded-2xl font-black text-2xl active:scale-95 transition-transform flex items-center justify-center uppercase tracking-widest shadow-xl bg-blue-600 hover:bg-blue-700 text-white"><CheckCircle className="w-8 h-8 mr-3" /> Registrar</button>
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full pb-40 md:pb-6 relative animate-in fade-in duration-300 w-full">
        ...
        <div className="w-full lg:w-2/3 flex flex-col gap-4 relative">
           ...
           <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
             ...
             <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-6">{_servicesConfig.map(s => <button key={s.id} onClick={() => {vibrate(); setOrderForm({...orderForm, serviceId: s.id, manualPrice: s.basePrice})}} className={`p-4 rounded-2xl border-2 font-bold text-left transition-all active:scale-95 flex flex-col justify-center min-w-0 ${orderForm.serviceId === s.id ? 'bg-blue-100 border-blue-500 text-blue-900 shadow-inner' : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-slate-300'}`}><p className="leading-tight text-sm md:text-base mb-1 truncate w-full">{s.name}</p><p className={`text-sm font-black num-font truncate w-full ${orderForm.serviceId === s.id ? 'text-blue-600' : 'text-slate-400'}`}>Q{formatMoney(Number(s.basePrice))}</p></button>)}{_servicesConfig.length === 0 && <div className="col-span-full p-8 bg-slate-50 rounded-2xl text-center text-slate-400 font-bold border-2 border-dashed border-slate-200">No hay servicios configurados. Agrégalos en Ajustes.</div>}</div>

             <div className="grid grid-cols-2 gap-4">

               <button onClick={() => {vibrate(); setNumpad({isOpen: true, field: 'bags', value: orderForm.bags.toString(), target: 'order'})}} className="flex flex-col items-center justify-center p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 active:bg-slate-200 transition-colors min-w-0"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 truncate w-full text-center">Bolsas o Piezas</span><span className="text-4xl md:text-5xl font-black text-slate-800 num-font tracking-tighter truncate w-full text-center">{orderForm.bags || '0'}</span></button>

               <button onClick={() => {vibrate(); setNumpad({isOpen: true, field: 'manualPrice', value: orderForm.manualPrice.toString(), target: 'order'})}} className="flex flex-col items-center justify-center p-6 rounded-[2rem] border-2 transition-colors min-w-0 bg-emerald-50 border-emerald-200 active:bg-emerald-100"><span className="text-[10px] font-black text-emerald-600/70 uppercase tracking-widest mb-1 truncate w-full text-center">Precio Cobrado</span>{orderForm.manualPrice == 0 ? <span className="text-lg md:text-xl font-black text-yellow-600 uppercase tracking-widest bg-yellow-100 px-4 py-2 rounded-xl mt-1">Por Definir</span> : <span className="text-4xl md:text-5xl font-black text-emerald-600 num-font tracking-tighter truncate w-full text-center">Q{orderForm.manualPrice}</span>}</button>

             </div>

           </div>

           <div className="flex bg-white p-2 rounded-[2rem] shadow-sm border border-slate-100 justify-between gap-1">{['Baja', 'Media', 'Alta'].map(p => <button key={p} onClick={() => {vibrate(); setOrderForm({...orderForm, priority: p})}} className={`flex-1 py-4 rounded-3xl font-black text-xs md:text-sm transition-all active:scale-95 truncate ${orderForm.priority === p ? (p==='Alta'?'bg-rose-500 text-white shadow-lg shadow-rose-200':'bg-slate-900 text-white shadow-lg') : 'text-slate-400 hover:bg-slate-50'}`}>{p}</button>)}</div>

           <div className="mt-4 w-full z-20"><button onClick={handleSaveOrder} className={`w-full py-6 md:py-8 rounded-[2rem] font-black text-xl md:text-2xl transition-all active:scale-95 flex items-center justify-center ${(!orderForm.clientId || !orderForm.serviceId || !orderForm.receivedBy || (!orderForm.bags || orderForm.bags === '0')) ? 'bg-slate-200 text-slate-400 shadow-none' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-xl shadow-blue-300'}`}><CheckCircle className="w-6 h-6 md:w-8 md:h-8 mr-3 shrink-0" /> REGISTRAR ORDEN</button></div>
  ...
  const renderOrdenes = () => {
    ...
    const statusColors = { 'Pendiente': 'bg-slate-100 text-slate-600', 'Lavado': 'bg-blue-100 text-blue-700', 'Secado': 'bg-orange-100 text-orange-700', 'Doblado': 'bg-purple-100 text-purple-700' };
    ...
    return (
      ...
      <div className="space-y-6 pb-24 md:pb-8 animate-in fade-in duration-300 w-full">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
          ...
          <div className="flex bg-slate-50 p-1.5 rounded-2xl w-full md:w-fit">{['activas', 'historial'].map(t => <button key={t} onClick={() => {vibrate(); setActiveOrderTab(t)}} className={`flex-1 px-8 py-3 rounded-xl font-black capitalize transition-all ${activeOrderTab === t ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>{t}</button>)}</div>

          <div className="relative w-full md:max-w-xs shrink-0 flex flex-col md:flex-row items-center gap-2">

            {currentUser?.role === 'employee' && <p className="text-xs font-bold text-slate-400 hidden md:block">Atajos: <kbd className="bg-slate-100 px-1 rounded">Alt+A</kbd> Activas, <kbd className="bg-slate-100 px-1 rounded">Alt+H</kbd> Historial. Usa <kbd className="bg-slate-100 px-1 rounded">↓</kbd> <kbd className="bg-slate-100 px-1 rounded">↑</kbd> para elegir.</p>}

            <div className="relative w-full">

              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/>

              <input ref={ordersSearchRef} type="text" placeholder={currentUser?.role === 'employee' ? "Buscar orden... (Alt+B)" : "Buscar cliente o # ticket..."} className="w-full bg-slate-50 py-3 px-12 rounded-2xl font-bold border-2 border-slate-50 outline-none focus:border-blue-500 transition-all text-sm placeholder:text-slate-400" value={orderSearchTerm} onChange={e => setOrderSearchTerm(e.target.value)} />

              {orderSearchTerm && <button onClick={() => {setOrderSearchTerm(''); ordersSearchRef.current?.focus();}} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500 font-bold">X</button>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          {sortedAndFilteredOrders.map((o, idx) => {
            ...
            return (
              ...
              <div id={`order-card-${idx}`} key={o.id} className={`bg-white p-6 rounded-[2.5rem] shadow-sm border-4 flex flex-col relative overflow-hidden transition-all ${selectedOrderIdx === idx && currentUser?.role === 'employee' ? 'border-blue-600 scale-[1.02] shadow-blue-200' : 'border-slate-100 hover:shadow-lg'}`}>
                ...
                <div className="flex justify-between items-start mb-6 gap-2 pr-16">
                  ...
                  {currentUser?.role === 'admin' && (
                    ...
                    <div className="absolute top-3 right-3 flex gap-1 shrink-0">
                ...
                <div className="flex justify-between items-center mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-100 gap-2">
                   ...
                   <div className="text-xs font-bold text-slate-500 space-y-2 min-w-0">
                      ...
                      <p className="flex items-center truncate"><Calendar className="w-4 h-4 mr-2 text-slate-400 shrink-0"/> Ingresó: {formatDateDisplay(o.requestDate)}</p>

                      {o.status === 'Entregado' ?

                         <p className="flex items-center truncate text-emerald-600"><CheckCircle className="w-4 h-4 mr-2 shrink-0"/> Entregó: {deliverer} ({formatDateDisplay(o.deliveryDate || o.paidDate)})</p> :
                   ...
                   <div className="text-right shrink-0">
                     ...
                     {o.total == 0 ? <p className="text-xs font-black text-yellow-600 bg-yellow-100 px-3 py-2 rounded-xl mb-1 uppercase">Por Definir</p> : <p className="text-xl font-black text-emerald-600 num-font leading-none mb-1">Q{formatMoney(Number(o.total))}</p>}
                ...
                <div className="mt-auto flex gap-3">
                  ...
                  <button onClick={() => requirePriceOrExecute(o, executeDeliver)} disabled={o.status === 'Entregado'} className={`w-full py-4 rounded-2xl font-black text-xs transition-all flex items-center justify-center ${o.status === 'Entregado' ? 'bg-slate-50 text-slate-300' : (selectedOrderIdx === idx && currentUser?.role === 'employee' ? 'bg-blue-600 text-white shadow-lg shadow-blue-300' : 'bg-slate-900 text-white shadow-lg shadow-slate-300')}`}>{o.status === 'Entregado' ? 'ENTREGADO' : (selectedOrderIdx === idx && currentUser?.role === 'employee' ? 'PRESIONA ENTER PARA COBRAR' : 'ENTREGAR Y COBRAR')}</button>
      ...
      <div className="space-y-8 pb-32 animate-in fade-in duration-300 w-full">
         ...
         {currentUser?.role === 'employee' ? (
           ...
           <div className="flex flex-col lg:flex-row gap-8 w-full h-full">
             ...
             <div className="w-full lg:w-1/3 bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col gap-4">

                <div className="flex items-center justify-center mb-2"><DollarSign className="w-12 h-12 text-emerald-500 bg-emerald-50 p-2 rounded-full" /></div>

                <h3 className="font-black text-2xl text-slate-800 text-center tracking-tight">Registro de Movimiento</h3>

                

                <select ref={txTypeRef} value={forms.transaction.type} onChange={e => {updateForm('transaction', 'type', e.target.value); updateForm('transaction', 'category', e.target.value === 'ENTRADA' ? (_txCategories.in[0] || '') : (_txCategories.out[0] || ''));}} onKeyDown={e => {if(e.key === 'Enter'){ e.preventDefault(); txDateRef.current?.focus();}}} className="w-full p-4 bg-slate-50 rounded-2xl border-2 border-slate-100 font-black outline-none focus:border-blue-500 text-slate-700 appearance-none text-center">

                  <option value="SALIDA">Gasto (Salida de Caja)</option>

                  <option value="ENTRADA">Ingreso (Entrada a Caja)</option>

                </select>

                

                <input ref={txDateRef} type="date" className="w-full p-4 bg-slate-50 rounded-2xl border-2 border-slate-100 font-bold outline-none focus:border-blue-500 text-slate-600" value={forms.transaction.date} onChange={e => updateForm('transaction', 'date', e.target.value)} onKeyDown={e => {if(e.key === 'Enter') { e.preventDefault(); txCategoryRef.current?.focus(); }}}/>

                <select ref={txCategoryRef} className="w-full p-4 bg-slate-50 rounded-2xl border-2 border-slate-100 font-bold outline-none focus:border-blue-500 text-slate-700 appearance-none" value={forms.transaction.category} onChange={e => {updateForm('transaction', 'category', e.target.value);}} onKeyDown={e => {if(e.key === 'Enter') { e.preventDefault(); txAmountRef.current?.focus(); }}}>

                   {forms.transaction.type === 'ENTRADA' ? _txCategories.in.map((c) => <option key={c} value={c}>{c}</option>) : _txCategories.out.map((c) => <option key={c} value={c}>{c}</option>)}

                </select>

                <input ref={txAmountRef} type="number" placeholder="Monto (Q)" className="w-full p-4 bg-slate-50 rounded-2xl border-2 border-slate-100 font-black text-2xl outline-none focus:border-blue-500 text-slate-700 text-center num-font" value={forms.transaction.amount} onChange={e => updateForm('transaction', 'amount', e.target.value)} onKeyDown={e => {if(e.key === 'Enter') { e.preventDefault(); txDescRef.current?.focus(); }}} />

                <input ref={txDescRef} type="text" placeholder="Comentario (Opcional)" className="w-full p-4 bg-slate-50 rounded-2xl border-2 border-slate-100 font-bold outline-none focus:border-blue-500 placeholder:text-slate-300" value={forms.transaction.desc} onChange={e => updateForm('transaction', 'desc', e.target.value)} onKeyDown={e => {if(e.key === 'Enter') { e.preventDefault(); txSubmitRef.current?.focus(); }}} />

                

                <button ref={txSubmitRef} onClick={() => {vibrate(); handleAddTransaction()}} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); handleAddTransaction(); } }} className="w-full py-5 font-black bg-blue-600 hover:bg-blue-700 text-white rounded-2xl active:scale-95 shadow-xl transition-transform mt-auto text-lg uppercase tracking-widest focus:ring-4 focus:ring-blue-300 outline-none">Registrar</button>

             </div>



             <div className="w-full lg:w-2/3 bg-white rounded-[3rem] border border-slate-100 shadow-sm flex flex-col overflow-hidden">

                <div className="p-6 bg-slate-50 border-b border-slate-100 font-black text-slate-800 flex items-center justify-between">

                   <span className="flex items-center"><Clock className="w-5 h-5 mr-2 text-slate-400"/>Tus Movimientos</span>

                </div>

                <div className="divide-y border-t-0 flex-1 overflow-y-auto no-scrollbar">
                     ...
                     return (
                       ...
                       <div key={t.id} className="p-6 flex justify-between items-center hover:bg-slate-50 transition-colors gap-4">
                         ...
                         <div className="flex items-center gap-3 shrink-0">
                           ...
                           <p className={`text-xl md:text-2xl font-black num-font ${isEntry ? 'text-emerald-600' : 'text-rose-600'}`}>{isEntry ? '+' : '-'}Q{formatMoney(t.amount)}</p>
                   ...
                   {filteredTransactions.length === 0 && <p className="p-8 text-center text-slate-400 font-bold">No has registrado movimientos en esta fecha.</p>}
  ...
  const renderReportesAnaliticos = () => {
    ...
    const categoriesIn = filteredTransactions.filter(t => t.type === 'ENTRADA').reduce((acc, t) => { acc[t.category] = (acc[t.category] || 0) + t.amount; return acc; }, {});

    const categoriesOut = filteredTransactions.filter(t => t.type === 'SALIDA').reduce((acc, t) => { acc[t.category] = (acc[t.category] || 0) + t.amount; return acc; }, {});
  ...
  const renderInsumos = () => {
    ...
    const filteredSupplies = supplySearchTerm ? _supplies.filter(s => s.name.toLowerCase().includes(supplySearchTerm.toLowerCase())) : _supplies;
      ...
      <div className="space-y-6 pb-24 md:pb-8 animate-in fade-in duration-300 w-full">
        ...
        <div className="bg-white rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-sm">
          ...
          {currentUser?.role === 'employee' && (
            ...
            <div className="relative w-full mb-6 flex flex-col md:flex-row gap-2 items-center">

              <div className="relative w-full">

                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400"/>

                <input ref={supplySearchRef} type="text" placeholder="Escribe para buscar... (Alt+B)" 

                  className="w-full bg-slate-50 py-4 px-12 rounded-2xl font-bold border-2 border-blue-100 outline-none focus:border-blue-500 transition-all text-lg"

                  value={supplySearchTerm} onChange={e => setSupplySearchTerm(e.target.value)}

                  onKeyDown={e => {

                    if (e.key === 'Enter' && filteredSupplies.length > 0) {

                      e.preventDefault();

                      const topSupply = filteredSupplies[0];

                      vibrate();

                      setForms(p => ({...p, restockSupply: {id: topSupply.id, name: topSupply.name, qty: '', cost: '', date: getLocalDateString()}}));
              ...
              <p className="text-xs font-bold text-slate-400 hidden md:block shrink-0">Usa <kbd className="bg-slate-100 px-1 rounded">↓</kbd> <kbd className="bg-slate-100 px-1 rounded">↑</kbd> y <kbd className="bg-slate-100 px-1 rounded">Enter</kbd> para comprar.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            ...
            {filteredSupplies.map((s, idx) => (
              ...
              <div id={`supply-card-${idx}`} key={s.id} className={`p-6 rounded-[2rem] border-4 transition-all flex flex-col min-w-0 ${selectedSupplyIdx === idx && currentUser?.role === 'employee' ? 'border-blue-600 scale-[1.02] shadow-blue-200' : (s.stock <= s.minAlert ? 'bg-rose-50 border-rose-200' : 'bg-slate-50 border-slate-100')}`}>
                ...
                <button onClick={() => {vibrate(); setForms(p => ({...p, restockSupply: {id: s.id, name: s.name, qty: '', cost: '', date: getLocalDateString()}})); toggleModal('restockSupply'); setTimeout(() => restockQtyRef.current?.focus(), 100);}} className={`w-full mt-2 text-white py-3 rounded-xl font-black text-sm active:scale-95 flex items-center justify-center shadow-lg transition-all ${selectedSupplyIdx === idx && currentUser?.role === 'employee' ? 'bg-blue-600' : 'bg-slate-900'}`}>

                  <ShoppingBag className="w-4 h-4 mr-2" /> {selectedSupplyIdx === idx && currentUser?.role === 'employee' ? 'Enter para Comprar' : 'Comprar'}
            ...
            {filteredSupplies.length === 0 && <p className="col-span-full text-slate-400 font-bold py-10 text-center">No hay insumos registrados con ese nombre.</p>}
    ...
    <div className="space-y-8 pb-32 animate-in fade-in duration-300 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col">
          ...
          <div className="flex justify-between items-center mb-8"><h3 className="font-black text-2xl flex items-center tracking-tight"><Store className="w-6 h-6 mr-3 text-emerald-600"/> Catálogo</h3><button onClick={() => {setForms(p=>({...p, service:{name:'', basePrice:''}})); toggleModal('service')}} className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 px-4 py-2 rounded-xl font-black text-xs uppercase tracking-widest active:scale-95 transition-colors">+ Añadir</button></div>
      ...
      <div className="bg-rose-50 p-8 md:p-10 rounded-[2.5rem] border border-rose-200 shadow-sm flex flex-col items-center justify-center text-center mt-10">
         ...
         <button onClick={() => {vibrate(); toggleModal('wipeData');}} className="bg-rose-600 hover:bg-rose-700 text-white px-8 py-4 rounded-2xl font-black active:scale-95 shadow-lg shadow-rose-300">Borrar Todo el Sistema</button>
  ...
  return (
    ...
    <>
      ...
      <div className="h-screen bg-slate-50 flex flex-col md:flex-row overflow-hidden relative">
          ...
          <div className="flex flex-row md:flex-col gap-1 md:gap-2 w-full justify-around md:justify-start h-full overflow-x-auto no-scrollbar">
            ...
            <div className="flex flex-row md:flex-col gap-1 md:gap-2 w-full justify-around md:justify-start">
              ...
              {[
                ...
                {id:'mostrador', label: 'Mostrador', icon: Store, role:['admin','employee'], shortcut: 'Alt+1'}, 

                {id:'ordenes', label: 'Órdenes', icon: ClipboardList, role:['admin','employee'], shortcut: 'Alt+2'}, 

                {id:'finanzas', label: 'Movimientos', icon: DollarSign, role:['admin', 'employee'], shortcut: 'Alt+3'},

                {id:'reportes', label: 'Reportes', icon: BarChart3, role:['admin']},

                {id:'insumos', label: 'Inventario', icon: Flame, role:['admin','employee'], shortcut: 'Alt+4'}, 
                ...
                <button key={item.id} onClick={() => {vibrate(); setActiveTab(item.id)}} className={`flex-1 md:flex-none flex flex-col lg:flex-row items-center justify-center lg:justify-start p-3 lg:p-4 rounded-[1.2rem] lg:rounded-2xl font-bold transition-all w-full md:w-auto lg:w-full min-w-[60px] shrink-0 ${activeTab === item.id ? 'bg-slate-900 text-white shadow-lg shadow-slate-300' : 'text-slate-400 hover:bg-slate-50'}`}>
                  ...
                  <item.icon className={`w-6 h-6 lg:mr-3 mb-1 lg:mb-0`}/>

                  <div className="flex flex-col items-center lg:items-start">

                    <span className="text-[10px] lg:text-sm tracking-wide">{item.label}</span>

                    {currentUser?.role === 'employee' && item.shortcut && (

                      <span className={`hidden lg:block text-[9px] px-1.5 py-0.5 rounded mt-1 ${activeTab === item.id ? 'bg-slate-700 text-slate-300' : 'bg-slate-200 text-slate-500'}`}>
        ...
        <main className="flex-1 flex flex-col h-[calc(100vh-75px)] md:h-screen overflow-hidden relative bg-slate-50/50">
          ...
          {activeTab !== 'mostrador' && activeTab !== 'ajustes' && (
            ...
            <header className="sticky top-0 z-30 bg-slate-50/90 backdrop-blur-md border-b border-slate-200/50 p-4 md:px-10 flex flex-col md:flex-row items-start md:items-center justify-between shrink-0 gap-4">
              ...
              <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 overflow-x-auto no-scrollbar w-full md:w-auto snap-x items-center relative">

                {currentUser?.role === 'employee' && <span className="hidden lg:block absolute -top-3 right-0 text-[9px] font-black text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full shadow-sm">Filtro: Alt+F</span>}
                ...
                {dateFilter.type === 'día' && (
                  ...
                  <div className="flex gap-2 ml-2 border-l pl-2 border-slate-200 shrink-0">
                    ...
                    <select ref={dateFilterRef} tabIndex={0} value={dateFilter.dayType || 'hoy'} onChange={e => setDateFilter({...dateFilter, dayType: e.target.value})} className="p-2 bg-slate-50 rounded-xl text-xs font-bold outline-none text-slate-600 cursor-pointer focus:border-blue-500 border-2 border-transparent">

                      <option value="hoy">Hoy</option><option value="ayer">Ayer</option><option value="custom">Específico</option>

                    </select>

                    {dateFilter.dayType === 'custom' && (<input tabIndex={0} type="date" className="p-2 bg-slate-50 rounded-xl text-xs font-bold outline-none text-slate-500 focus:border-blue-500 border-2 border-transparent" value={dateFilter.specificDate} onChange={e => setDateFilter({...dateFilter, specificDate: e.target.value})} title="Fecha" />)}
                ...
                {dateFilter.type === 'mes' && (
                  ...
                  <div className="flex gap-2 ml-2 border-l pl-2 border-slate-200 shrink-0">
                    ...
                    <select ref={dateFilterRef} tabIndex={0} value={dateFilter.month} onChange={e => setDateFilter({...dateFilter, month: parseInt(e.target.value)})} className="p-2 bg-slate-50 rounded-xl text-xs font-bold outline-none text-slate-600 cursor-pointer focus:border-blue-500 border-2 border-transparent">{MONTHS.map((m, i) => <option key={m} value={i+1}>{m}</option>)}</select>

                    <select tabIndex={0} value={dateFilter.year} onChange={e => setDateFilter({...dateFilter, year: parseInt(e.target.value)})} className="p-2 bg-slate-50 rounded-xl text-xs font-bold outline-none text-slate-600 cursor-pointer focus:border-blue-500 border-2 border-transparent">{YEARS.map(y => <option key={y} value={y}>{y}</option>)}</select>
                ...
                {dateFilter.type === 'año' && (
                  ...
                  <div className="flex gap-2 ml-2 border-l pl-2 border-slate-200 shrink-0">
                    ...
                    <select ref={dateFilterRef} tabIndex={0} value={dateFilter.year} onChange={e => setDateFilter({...dateFilter, year: parseInt(e.target.value)})} className="p-2 bg-slate-50 rounded-xl text-xs font-bold outline-none text-slate-600 cursor-pointer focus:border-blue-500 border-2 border-transparent">{YEARS.map(y => <option key={y} value={y}>{y}</option>)}</select>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 scroll-smooth">
            ...
            <div className="w-full min-h-full">
              ...
              {activeTab === 'mostrador' && (currentUser?.role === 'admin' ? renderMostradorTouch() : renderMostradorLaptop())}
        ...
        {/* --- TECLADO UNIVERSAL BLINDADO (Z-INDEX 9999, SOLO ADMIN) --- */}
            ...
            <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
              ...
              <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
                ...
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-4">
                  ...
                  <p className="flex justify-between"><span className="text-slate-400 font-bold">Ingreso:</span><span className="font-black text-slate-700">{formatDateDisplay(o.requestDate)}</span></p>

                  <p className="flex justify-between"><span className="text-slate-400 font-bold">Entrega:</span><span className="font-black text-slate-700">{o.deliveryDate || 'Pendiente'}</span></p>
                ...
                <button autoFocus onClick={() => toggleModal('viewOrder')} className="w-full py-5 font-black bg-slate-900 text-white rounded-2xl active:scale-95 shadow-lg">Cerrar</button>
        ...
        {modals.transaction && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input ref={modalTxDateRef} type="date" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none text-slate-600 focus:border-blue-500" value={forms.transaction.date} onChange={e => updateForm('transaction', 'date', e.target.value)} onKeyDown={e => e.key === 'Enter' && modalTxCategoryRef.current?.focus()} />

              

              {currentUser?.role === 'employee' ? (

                <input ref={modalTxAmountRef} type="number" autoFocus placeholder="Monto (Q)" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-2xl outline-none focus:border-blue-600 num-font text-center" value={forms.transaction.amount} onChange={e => updateForm('transaction', 'amount', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); modalTxCategoryRef.current?.focus();}}} />
              ...
              <select ref={modalTxCategoryRef} className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none text-slate-700 appearance-none focus:border-blue-500" value={forms.transaction.category} onChange={e => {updateForm('transaction', 'category', e.target.value); modalTxDescRef.current?.focus()}} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); modalTxDescRef.current?.focus();}}}>{forms.transaction.type === 'ENTRADA' ? _txCategories.in.map((c:string) => <option key={c} value={c}>{c}</option>) : _txCategories.out.map((c:string) => <option key={c} value={c}>{c}</option>)}</select>

              <input ref={modalTxDescRef} type="text" placeholder="Concepto (Opcional)" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none placeholder:text-slate-300 focus:border-blue-500" value={forms.transaction.desc} onChange={e => updateForm('transaction', 'desc', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleAddTransaction();}}} />

              <div className="flex gap-4 pt-4"><button onClick={() => toggleModal('transaction')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => {vibrate(); handleAddTransaction()}} className="flex-1 py-5 font-black bg-slate-900 text-white rounded-2xl active:scale-95 shadow-lg transition-transform">Guardar</button></div>
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95 border-2 border-rose-100">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button autoFocus onClick={() => {vibrate(); setTransactions(_transactions.filter(x => x.id !== forms.deleteTx.id)); toggleModal('deleteTx'); showMsg("Movimiento eliminado", false); }} className="flex-1 py-5 font-black bg-rose-600 text-white rounded-2xl active:scale-95 shadow-lg shadow-rose-200 transition-transform">Sí, Eliminar</button>
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95 border-2 border-rose-100">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button autoFocus onClick={() => {vibrate(); const o = forms.deleteOrder.order; if (o.status === 'Entregado') revertOrderEffects(o); setOrders(_orders.filter(x => x.id !== o.id)); toggleModal('deleteOrder'); showMsg("Orden eliminada", false); }} className="flex-1 py-5 font-black bg-rose-600 text-white rounded-2xl active:scale-95 shadow-lg shadow-rose-200 transition-transform">Eliminar Orden</button>
        ...
        {modals.category && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input autoFocus type="text" placeholder="Nombre" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500" value={forms.category.name} onChange={e => updateForm('category', 'name', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleAddCategory();}}} />

              <div className="flex gap-4 pt-4"><button onClick={() => toggleModal('category')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => {vibrate(); handleAddCategory()}} className={`flex-1 py-5 font-black text-white rounded-2xl active:scale-95 shadow-lg transition-transform ${forms.category.type === 'ENTRADA' ? 'bg-emerald-600' : 'bg-rose-600'}`}>Agregar</button></div>
        ...
        {modals.editCategory && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input autoFocus type="text" placeholder="Nombre" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500" value={forms.editCategory.newName} onChange={e => updateForm('editCategory', 'newName', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleEditCategory();}}} />

              <div className="flex gap-4 pt-4"><button onClick={() => toggleModal('editCategory')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => {vibrate(); handleEditCategory()}} className={`flex-1 py-5 font-black text-white rounded-2xl active:scale-95 shadow-lg transition-transform ${forms.editCategory.type === 'ENTRADA' ? 'bg-emerald-600' : 'bg-rose-600'}`}>Actualizar</button></div>
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95 max-h-[90vh] overflow-y-auto no-scrollbar">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 max-h-40 overflow-y-auto no-scrollbar touch-pan-y">
                  ...
                  {_employees.map(e => <button key={e.id} onClick={() => {vibrate(); updateForm('deliverOrder', 'deliveredBy', e.id.toString()); setTimeout(() => deliverSubmitRef.current?.focus(), 50); }} className={`p-4 rounded-2xl border-2 font-black transition-all active:scale-95 outline-none focus:border-blue-500 ${forms.deliverOrder.deliveredBy === e.id.toString() ? 'bg-slate-900 border-slate-900 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>{e.name}</button>)}

                </div>

              </div>

              <div className="flex gap-4 pt-4"><button onClick={() => toggleModal('deliverOrder')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Volver</button><button ref={deliverSubmitRef} onClick={() => {vibrate(); confirmDelivery()}} onKeyDown={e => {if(e.key==='Enter'){e.preventDefault(); confirmDelivery();}}} disabled={!forms.deliverOrder.deliveredBy} className="flex-1 py-5 bg-emerald-600 text-white rounded-2xl font-black disabled:opacity-30 active:scale-95 shadow-lg transition-transform">Confirmar</button></div>
        ...
        {modals.supply && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input ref={supplyNameRef} type="text" placeholder="Ej. Jabón Galón" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500" value={forms.supply.name} onChange={e => updateForm('supply', 'name', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); supplyStockRef.current?.focus();}}} />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {currentUser?.role === 'employee' ? (
                  ...
                  <>
                    ...
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-2 block">Inicial</label><input ref={supplyStockRef} type="number" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl outline-none focus:border-blue-500 num-font" value={forms.supply.stock} onChange={e => updateForm('supply', 'stock', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); supplyMinRef.current?.focus();}}} /></div>

                    <div><label className="text-[10px] font-black uppercase text-rose-500 ml-2 mb-2 block">Min:</label><input ref={supplyMinRef} type="number" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl text-rose-600 outline-none focus:border-rose-500 num-font" value={forms.supply.minAlert} onChange={e => updateForm('supply', 'minAlert', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleAddSupply();}}} /></div>
        ...
        {modals.editSupply && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input ref={supplyNameRef} type="text" placeholder="Nombre" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500" value={forms.editSupply.name} onChange={e => updateForm('editSupply', 'name', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); supplyStockRef.current?.focus();}}}/>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {currentUser?.role === 'employee' ? (
                  ...
                  <>
                    ...
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-2 block">Actual</label><input ref={supplyStockRef} type="number" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl outline-none focus:border-blue-500 num-font" value={forms.editSupply.stock} onChange={e => updateForm('editSupply', 'stock', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); supplyMinRef.current?.focus();}}} /></div>

                    <div><label className="text-[10px] font-black uppercase text-rose-500 ml-2 mb-2 block">Min:</label><input ref={supplyMinRef} type="number" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl text-rose-600 outline-none focus:border-rose-500 num-font" value={forms.editSupply.minAlert} onChange={e => updateForm('editSupply', 'minAlert', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleEditSupply();}}} /></div>
              ...
              <div className="flex gap-4 pt-6"><button onClick={() => toggleModal('editSupply')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => {vibrate(); handleEditSupply()}} className="flex-1 py-5 font-black bg-blue-600 text-white rounded-2xl active:scale-95 shadow-lg transition-transform">Actualizar</button></div>
        ...
        {modals.restockSupply && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {currentUser?.role === 'employee' ? (
                  ...
                  <>
                    ...
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-2 block">Cant. Comprada</label><input ref={restockQtyRef} type="number" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl outline-none focus:border-blue-500 num-font" value={forms.restockSupply.qty} onChange={e => updateForm('restockSupply', 'qty', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); restockCostRef.current?.focus();}}} /></div>

                    <div><label className="text-[10px] font-black uppercase text-rose-500 ml-2 mb-2 block">Total (Q)</label><input ref={restockCostRef} type="number" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl text-rose-600 outline-none focus:border-rose-500 num-font" value={forms.restockSupply.cost} onChange={e => updateForm('restockSupply', 'cost', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleRestockSupply();}}} /></div>
              ...
              <div className="flex gap-4 pt-6"><button onClick={() => toggleModal('restockSupply')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => {vibrate(); handleRestockSupply()}} className="flex-1 py-5 font-black bg-slate-900 text-white rounded-2xl active:scale-95 shadow-lg shadow-slate-300 transition-transform">Registrar Compra</button></div>
        ...
        {modals.client && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input autoFocus type="text" placeholder="Nombre completo" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500" value={forms.client.name} onChange={e => updateForm('client', 'name', e.target.value)} onKeyDown={e => {if(e.key==='Enter'){e.preventDefault(); e.target.nextElementSibling.focus();}}} />

              <input type="tel" placeholder="Celular (Opcional)" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500 num-font" value={forms.client.phone} onChange={e => updateForm('client', 'phone', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleAddClient();}}} />

              <div className="flex gap-4 pt-6"><button onClick={() => toggleModal('client')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => {vibrate(); handleAddClient()}} className="flex-1 py-5 font-black bg-blue-600 text-white rounded-2xl active:scale-95 shadow-lg shadow-blue-200 transition-transform">Guardar</button></div>
        ...
        {modals.editClient && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input autoFocus type="text" placeholder="Nombre completo" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500" value={forms.editClient.name} onChange={e => updateForm('editClient', 'name', e.target.value)} onKeyDown={e => {if(e.key==='Enter'){e.preventDefault(); e.target.nextElementSibling.focus();}}} />

              <input type="tel" placeholder="Celular (Opcional)" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500 num-font" value={forms.editClient.phone} onChange={e => updateForm('editClient', 'phone', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleEditClient();}}} />

              <div className="flex gap-4 pt-6"><button onClick={() => toggleModal('editClient')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => {vibrate(); handleEditClient()}} className="flex-1 py-5 font-black bg-blue-600 text-white rounded-2xl active:scale-95 shadow-lg transition-transform">Actualizar</button></div>
        ...
        {modals.employee && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input autoFocus type="text" placeholder="Nombre Corto (Ej. María)" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-blue-500" value={forms.employee.name} onChange={e => updateForm('employee', 'name', e.target.value)} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); vibrate(); if(!forms.employee.name) return; setEmployees([{id: Date.now(), ...forms.employee}, ..._employees]); setForms(p=>({...p, employee:{name:'',phone:''}})); toggleModal('employee'); showMsg("Empleado activo", false); }}} />

              <div className="flex gap-4 pt-6"><button onClick={() => toggleModal('employee')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => { vibrate(); if(!forms.employee.name) return; setEmployees([{id: Date.now(), ...forms.employee}, ..._employees]); setForms(p=>({...p, employee:{name:'',phone:''}})); toggleModal('employee'); showMsg("Empleado activo", false); }} className="flex-1 py-5 bg-blue-600 text-white rounded-2xl font-black active:scale-95 shadow-lg shadow-blue-200 transition-transform">Ingresar</button></div>
        ...
        {modals.service && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input autoFocus type="text" placeholder="Ej. Lavado de Edredón" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-emerald-500" value={forms.service.name} onChange={e => updateForm('service', 'name', e.target.value)} onKeyDown={e => {if(e.key==='Enter'){e.preventDefault(); e.target.nextElementSibling?.focus();}}}/>

              

              {currentUser?.role === 'employee' ? (

                <input type="number" placeholder="Precio Base (Q)" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-2xl outline-none focus:border-emerald-500 num-font mt-6 text-center" value={forms.service.basePrice} onChange={e => updateForm('service', 'basePrice', e.target.value)} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); vibrate(); if(!forms.service.name || !forms.service.basePrice) return; setServicesConfig([..._servicesConfig, {id: Date.now(), ...forms.service}]); setForms(p=>({...p, service:{name:'',basePrice:''}})); toggleModal('service'); showMsg("Catálogo guardado", false); } }} />

              ) : (

                <div onClick={() => {vibrate(); setNumpad({isOpen: true, field: 'basePrice', value: forms.service.basePrice, target: 'service'})}} className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-2xl cursor-pointer num-font mt-6 text-center">
              ...
              <div className="flex gap-4 pt-6"><button onClick={() => toggleModal('service')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => { vibrate(); if(!forms.service.name || !forms.service.basePrice) return; setServicesConfig([..._servicesConfig, {id: Date.now(), ...forms.service}]); setForms(p=>({...p, service:{name:'',basePrice:''}})); toggleModal('service'); showMsg("Catálogo guardado", false); }} className="flex-1 py-5 bg-emerald-600 text-white rounded-2xl font-black active:scale-95 shadow-lg shadow-emerald-200 transition-transform">Crear</button></div>
        ...
        {modals.editService && (
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <input autoFocus type="text" placeholder="Ej. Lavado de Edredón" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-bold outline-none focus:border-emerald-500" value={forms.editService.name} onChange={e => updateForm('editService', 'name', e.target.value)} onKeyDown={e => {if(e.key==='Enter'){e.preventDefault(); e.target.nextElementSibling?.focus();}}}/>

              

              {currentUser?.role === 'employee' ? (

                <input type="number" placeholder="Precio Base (Q)" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-2xl outline-none focus:border-emerald-500 num-font mt-6 text-center" value={forms.editService.basePrice} onChange={e => updateForm('editService', 'basePrice', e.target.value)} onKeyDown={e => { if(e.key === 'Enter') { e.preventDefault(); vibrate(); if(!forms.editService.name || !forms.editService.basePrice) return; setServicesConfig(_servicesConfig.map(s => s.id === forms.editService.id ? {...s, name: forms.editService.name, basePrice: forms.editService.basePrice} : s)); toggleModal('editService'); showMsg("Servicio actualizado", false); } }} />

              ) : (

                <div onClick={() => {vibrate(); setNumpad({isOpen: true, field: 'basePrice', value: forms.editService.basePrice, target: 'editService'})}} className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-2xl cursor-pointer num-font mt-6 text-center">

                  {forms.editService.basePrice ? `Q ${forms.editService.basePrice}` : <span className="text-slate-300">Precio Base (Q)</span>}
              ...
              <div className="flex gap-4 pt-6"><button onClick={() => toggleModal('editService')} className="flex-1 py-5 font-black text-slate-500 bg-slate-100 rounded-2xl active:scale-95 transition-transform">Cancelar</button><button onClick={() => { vibrate(); if(!forms.editService.name || !forms.editService.basePrice) return; setServicesConfig(_servicesConfig.map(s => s.id === forms.editService.id ? {...s, name: forms.editService.name, basePrice: forms.editService.basePrice} : s)); toggleModal('editService'); showMsg("Servicio actualizado", false); }} className="flex-1 py-5 bg-emerald-600 text-white rounded-2xl font-black active:scale-95 shadow-lg shadow-emerald-200 transition-transform">Actualizar</button></div>
          ...
          <div className="fixed inset-0 z-[1000] bg-slate-900/40 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in">
            ...
            <div className="bg-white w-full max-w-md rounded-t-[2.5rem] md:rounded-[3rem] p-8 md:p-10 space-y-6 shadow-2xl animate-in slide-in-from-bottom-10 md:zoom-in-95">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {currentUser?.role === 'employee' ? (
                  ...
                  <>
                    ...
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-2 block">Bolsas</label><input type="number" autoFocus className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl outline-none focus:border-blue-500 num-font text-center" value={forms.editOrder.bags} onChange={e => updateForm('editOrder', 'bags', e.target.value)} onKeyDown={e => {if(e.key==='Enter'){e.preventDefault(); e.target.parentElement.nextElementSibling.lastChild.focus();}}}/></div>

                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-2 block">Monto Q</label><input type="number" className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl text-emerald-600 outline-none focus:border-emerald-500 num-font text-center" value={forms.editOrder.manualPrice} onChange={e => updateForm('editOrder', 'manualPrice', e.target.value)} onKeyDown={e => {if(e.key === 'Enter'){e.preventDefault(); handleEditOrder();}}} /></div>
                  ...
                  <>
                    ...
                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-2 block">Bolsas/Pzas</label><div onClick={() => {vibrate(); setNumpad({isOpen: true, field: 'bags', value: forms.editOrder.bags.toString(), target: 'editOrder'})}} className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl text-slate-700 cursor-pointer num-font text-center">{forms.editOrder.bags || '0'}</div></div>

                    <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-2 block">Monto (Q)</label><div onClick={() => {vibrate(); setNumpad({isOpen: true, field: 'manualPrice', value: forms.editOrder.manualPrice.toString(), target: 'editOrder'})}} className="w-full p-5 bg-slate-50 rounded-2xl border-2 font-black text-xl text-emerald-600 cursor-pointer num-font text-center">{forms.editOrder.manualPrice || '0'}</div></div>
              ...
              <div>
                ...
                <select value={forms.editOrder.status} onChange={(e) => updateForm('editOrder', 'status', e.target.value)} className="w-full p-5 bg-slate-50 rounded-2xl border-2 border-slate-100 font-bold outline-none text-slate-700 cursor-pointer focus:border-blue-500"><option value="Pendiente">Pendiente</option><option value="Lavado">Lavado</option><option value="Secado">Secado</option><option value="Doblado">Doblado</option><option value="Entregado">Entregado</option></select>
    ...
    if (this.state.hasError) {
      ...
      return (
        ...
        <div className="h-screen bg-rose-50 flex flex-col items-center justify-center p-10 text-center">
          ...
          <button onClick={() => window.location.reload()} className="mt-8 bg-rose-600 text-white px-8 py-4 rounded-xl font-black shadow-lg transition-transform active:scale-95">Reiniciar Sistema</button>
